...
下一动作：草稿 PR [#1](https://github.com/aswansong/macwin/pull/1) 在提交 `ab234f8` 上已完成静态审查且无评论，GitHub Actions run `30761380120` 的 macOS 15 与 Windows 检查均通过；现在等待负责人提供真实 Windows 10/11 与 Apple 芯片 macOS 15/26 设备、签名凭据和一次性人工验收，完成权限/外设/安装卸载矩阵；OD-005/OD-007 未关闭前不新增真实 Wi‑Fi 密码或自动安装实现。
