...
负责人已授权从 Alpha 0.2 推进到可供普通 Windows 新手使用的 v1.0.0：允许在本分支实现剩余 P0、加入经过审查的生产依赖与 Tauri 插件、构建安装包、创建 PR、合并 `main`、打标签和创建 GitHub Release。未签名构建不得作为正式版本发布。
...
3. P0-009/P0-010 已扩展 Windows 白名单、官方入口和包规则；可信自动安装、签名/哈希和 Homebrew 独立确认仍未完成。
4. 完成 P0-011 Wi‑Fi 个人网络安全链路；若平台接口不能同时满足安全不变量，集中记录为人类决策门，不改用不安全接口。
5. P0-013 已加入本地 HTML/脱敏 JSON 报告；P0-014 已加入 updater 依赖、endpoint 和签名 Release 草稿，公钥注入、错误日志和 GitHub 入口仍需完成。
...
下一动作：补齐软件/开发模块的安全安装状态机和本地错误日志，再做 Wi‑Fi 安全决策门审查。任何需要真实设备、系统弹窗或签名凭据的事项先完成不受影响工作，集中列入人类门槛清单。
...
- 主控可见可用额度低于 20% 时停止派发新任务并保留 Goal，不把暂停写成完成或阻塞。
