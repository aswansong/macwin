...
## prototype/visual-system-v2 视觉分支补充

| 能力 | 浏览器证据 | 边界 |
| --- | --- | --- |
| Windows W1 → W2 → W3 | 本地 Vite 预览实际点击、通行证模块联动、导出前复核和生成后交接 | 只使用虚构数据，不证明 Windows 扫描或真实文件生成 |
| Mac B → C | 本地 Vite 预览实际点击导入、强制计划、执行、结果、报告、指南和恢复 | 不证明真实 `.habitpack` 导入或目标端系统修改 |
| 异常与安全状态表达 | UAC 拒绝、Mac 权限拒绝、离线、第三方拒绝、单模块失败、损坏包均可复现 | 权限、Wi‑Fi 密码、第三方安装、签名和真实设备仍受原有门槛约束 |
| 响应式与可访问性 | 1440×900、1280×720、390×844；可见焦点、减少动态效果 CSS、窄窗无横向溢出 | 200% 系统文字缩放仍需负责人按真实桌面环境复核 |
