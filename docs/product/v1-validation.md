...
| 发布资产整体审计 | `scripts/validate-release-assets.py` 的 3 条离线夹具测试；Release publish job 在发布前校验双平台 SHA-256、SBOM、安装包、updater 签名文本和 `latest.json` 一致性 | 只验证已下载产物的结构与元数据，不替代代码签名、公证、stapling 或真实设备安装验收 |
