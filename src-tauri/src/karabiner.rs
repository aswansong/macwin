...
pub fn plan_for_target() -> KeyboardCompatibilityPlan {
    ...
    let has_external = devices
        .iter()
        .any(|device| device.kind == "external" && device.recognized);
    KeyboardCompatibilityPlan {
        built_in_enabled: devices
            .iter()
    ...
    #[cfg(target_os = "macos")]
    {
        let Some(path) = config_path() else {
            return Ok(ApplyReport {
                ...
                detail: "未找到 Karabiner-Elements 配置；请从官方入口安装后重试".to_owned(),
            });
        };
        if !path.exists() {
            return Ok(ApplyReport {
                status: "manual_action_required".to_owned(),
                detail:
                    "Karabiner-Elements 已安装，但配置尚未生成；请打开一次官方应用并授予所需权限"
                        .to_owned(),
            });
        }
        let bytes = std::fs::read(&path).map_err(|_| PlatformError::Read("KARABINER_READ"))?;
        let mut value: Value =
            serde_json::from_slice(&bytes).map_err(|_| PlatformError::Read("KARABINER_JSON"))?;
        let rules = rules_for_request(request);
        if rules.is_empty() {
            return Ok(ApplyReport {
                status: "manual_action_required".to_owned(),
                detail: "没有可安全匹配的内置或外接键盘；MacWin 不会猜测设备".to_owned(),
            });
        }
        let changed = merge_managed_rules(&mut value, &rules)
            .map_err(|_| PlatformError::Write("KARABINER_STRUCTURE"))?;
        if changed {
            write_config_atomically(&path, &bytes, &value)?;
        }
        let verified_bytes =
            std::fs::read(&path).map_err(|_| PlatformError::Read("KARABINER_VERIFY"))?;
        let verified: Value = serde_json::from_slice(&verified_bytes)
            .map_err(|_| PlatformError::Read("KARABINER_VERIFY"))?;
        if !rules.iter().all(|rule| {
            rule.get("description")
                .and_then(Value::as_str)
                .is_some_and(|description| has_managed_description(&verified, description))
        }) {
            return Err(PlatformError::Read("KARABINER_VERIFY"));
        }
        Ok(ApplyReport {
            status: "applied_verified".to_owned(),
            detail: "选择性 Ctrl 规则已写入，终端/远程桌面/虚拟机例外保留真实 Ctrl".to_owned(),
        })
    }
    ...
    #[cfg(target_os = "macos")]
    {
        let Some(path) = config_path() else {
            return Ok(ApplyReport {
                ...
                detail: "没有可移除的 MacWin 规则".to_owned(),
            });
        };
        if !path.exists() {
            return Ok(ApplyReport {
        ...
        let mut value: Value =
            serde_json::from_slice(&bytes).map_err(|_| PlatformError::Read("KARABINER_JSON"))?;
        if remove_managed_rules(&mut value)
            .map_err(|_| PlatformError::Write("KARABINER_STRUCTURE"))?
        {
            write_config_atomically(&path, &bytes, &value)?;
        }
        ...
        let verified: Value = serde_json::from_slice(&verified_bytes)
            .map_err(|_| PlatformError::Read("KARABINER_VERIFY"))?;
        if managed_descriptions()
            .iter()
            .any(|description| has_managed_description(&verified, description))
        {
            return Err(PlatformError::Read("KARABINER_VERIFY"));
        }
        Ok(ApplyReport {
            status: "rolled_back_verified".to_owned(),
            detail: "只移除了 MacWin v1 规则".to_owned(),
        })
    }
    #[cfg(not(target_os = "macos"))]
    {
    ...
    config
        ...
        .any(|profile| {
            profile
                ...
                .into_iter()
                .flatten()
                .any(|rule| rule.get("description").and_then(Value::as_str) == Some(description))
        })
}
...
fn rules_for_request(request: &KeyboardCompatibilityRequest) -> Vec<Value> {
    ...
    if request.built_in_enabled {
        let identifiers = request
            .devices
            .iter()
            .filter(|device| device.kind == "built_in" && device.recognized)
            .map(device_identifier)
            .collect::<Vec<_>>();
        if !identifiers.is_empty() {
            rules.push(make_rule(INTERNAL_DESCRIPTION, identifiers));
        }
    }
    if request.external_enabled {
        let identifiers = request
            .devices
            .iter()
            .filter(|device| device.kind == "external" && device.recognized)
            .map(device_identifier)
            .collect::<Vec<_>>();
        if !identifiers.is_empty() {
            rules.push(make_rule(EXTERNAL_DESCRIPTION, identifiers));
        }
    }
    rules
}
...
fn device_identifier(device: &KeyboardDevice) -> Value {
    ...
    if let Some(vendor_id) = device.vendor_id {
        identifier.insert("vendor_id".to_owned(), Value::from(vendor_id));
    }
    if let Some(product_id) = device.product_id {
        identifier.insert("product_id".to_owned(), Value::from(product_id));
    }
    if let Some(location_id) = device.location_id {
        identifier.insert("location_id".to_owned(), Value::from(location_id));
    }
...
fn config_path() -> Option<PathBuf> {
    home_dir().map(|home| home.join(".config/karabiner/karabiner.json"))
}
...
fn write_config_atomically(
    ...
    if !backup.exists() {
        std::fs::write(&backup, original).map_err(|_| PlatformError::Write("KARABINER_BACKUP"))?;
    }
    let temp = path.with_file_name("karabiner.json.macwin.tmp");
    let encoded =
        serde_json::to_vec_pretty(value).map_err(|_| PlatformError::Write("KARABINER_ENCODE"))?;
    let file = std::fs::OpenOptions::new()
        .write(true)
        .create_new(true)
        .open(&temp)
        .map_err(|_| PlatformError::Write("KARABINER_TEMP"))?;
    use std::io::Write;
    let mut file = file;
    file.write_all(&encoded)
        .map_err(|_| PlatformError::Write("KARABINER_TEMP"))?;
    file.sync_all()
        .map_err(|_| PlatformError::Write("KARABINER_SYNC"))?;
...
fn detect_status() -> KarabinerStatus {
    let candidates = [
        ...
        home_dir()
            .map(|home| home.join("Applications/Karabiner-Elements.app"))
            .unwrap_or_default(),
    ];
    ...
    let version = app
        .and_then(|path| {
            std::process::Command::new("/usr/bin/defaults")
                .arg("read")
                .arg(format!("{}/Contents/Info.plist", path.display()))
    ...
    KarabinerStatus {
        ...
        version,
        config_present: config_path().is_some_and(|path| path.exists()),
        permission: "未自动探测；首次应用请按系统提示授权".to_owned(),
        official_url: "https://karabiner-elements.pqrs.org/".to_owned(),
    }
}
...
fn detect_devices() -> Vec<KeyboardDevice> {
    let output = std::process::Command::new("/usr/bin/hidutil")
        ...
        .output()
        .ok();
    let Some(output) = output.and_then(|out| {
        if out.status.success() {
    ...
    output
        ...
        .filter_map(|(index, line)| {
            ...
            if lower.contains("virtualhidkeyboard") {
                return None;
            }
            ...
            if parse_number(columns[3]) != Some(1) || parse_number(columns[4]) != Some(6) {
                return None;
            }
            ...
            let built_in = columns.last().is_some_and(|value| *value == "1")
                || lower.contains("internal keyboard");
            ...
            let recognized = (built_in && location_id.is_some_and(|value| value != 0))
            ...
            let raw = format!(
                ...
                location_id.unwrap_or(index as u64),
                kind
            );
...
mod tests {
    ...
    fn merge_preserves_user_rules_and_fixed_exceptions() {
        let device = KeyboardDevice {
            name: "Apple Internal".into(),
            kind: "built_in".into(),
            recognized: true,
            redacted_id: "kb-test".into(),
            vendor_id: Some(1),
            product_id: Some(2),
            location_id: Some(3),
        };
        let request = KeyboardCompatibilityRequest {
        ...
        assert!(rules
            ...
            .unwrap()
            .iter()
            .any(|rule| rule["description"] == "User rule"));
        assert_eq!(
            rules
                .as_array()
                .unwrap()
                .iter()
                .filter(|rule| rule["description"] == INTERNAL_DESCRIPTION)
                .count(),
            1
        );
        assert_eq!(
            rules[1]["conditions"][1]["type"],
            "frontmost_application_unless"
        );
        assert_eq!(
            rules[1]["manipulators"].as_array().unwrap().len(),
            SHORTCUTS.len()
        );
    }
    ...
    fn device_serialization_exposes_only_redacted_identifier() {
        let device = KeyboardDevice {
            name: "MacBook 内置键盘".into(),
            kind: "built_in".into(),
            recognized: true,
            redacted_id: "kb-test".into(),
            vendor_id: Some(123),
            product_id: Some(456),
            location_id: Some(789),
        };
