...
mod tests {
    ...
    #[test]
    #[ignore = "requires the user-provided external package path"]
    fn parses_user_provided_external_regression_package() {
        let path = std::env::var("MACWIN_REAL_HABITPACK")
            .expect("MACWIN_REAL_HABITPACK must point to the external regression package");
        let parsed = parse_file(path).expect("external package must pass the frozen parser");
        assert_eq!(parsed.entries, 6);
        assert!(parsed.keyboard.is_some());
        assert!(parsed.pointer.is_some());
        assert_eq!(parsed.software.len(), 4);
        assert!(parsed.software.iter().any(|item| item.id == "git"));
        assert!(!parsed.contains_secrets);
    }
}
