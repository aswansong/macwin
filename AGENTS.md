...
- 当前只允许完善产品文档、规格、决策、执行计划和验收标准。
- 不创建 Tauri 应用代码、交互原型、系统扫描脚本或配置执行器，除非 `docs/execution/active-plan.md` 明确进入对应阶段且负责人再次授权。
- 交互原型未来必须位于独立的 `prototype/ui-flow-v1` 分支，明确标记为可丢弃代码；不得直接合并进正式实现。
