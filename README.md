...
当前里程碑计划交付完整可点击流程原型和闭合的 `.habitpack` 格式规范。本轮只加固独立的 `prototype/ui-flow-v2-hardening` 分支；M1 尚未完成，`.habitpack` 的事实源与格式资产仍需后续在 `main` 闭环。原型只使用虚构数据，不扫描、不提权、不联网、不修改系统，也不会直接合入正式实现。生产应用、真实平台适配和安装包仍未开始。[D-024、D-027]
...
打开终端显示的本地地址。首页只提供“开始兼容检测”和“在 Mac 导入配置”两个入口；进入流程后，每个平台只显示 3 个用户阶段。默认收起的“原型控制”可切换场景、计划信息量和减少动态效果；也可使用 `?scenario=offline` 等 URL 参数。场景包括正常完成、UAC 被拒绝、Mac 权限被拒绝、无网待重试、单模块失败、第三方工具被拒绝、需要手动完成、恢复演示，以及损坏/不兼容包阻断。`?text=200` 仅用于原型的 200% 文字缩放检查。

“鼠标与触控板滚动”可以在 Windows 选择页预选，但不代表同意安装 LinearMouse。Mac 强制计划页会始终提供独立、默认未勾选的候选工具确认和待验证披露；不确认仍可继续，只有指针动作稍后处理。结果、报告、指南和恢复在一个“操作习惯”模块内分别显示内置键盘 Ctrl、外接 Windows 键盘 Ctrl、鼠标与触控板滚动三个动作。[D-007、D-010、D-011、D-026、OD-007]
...
自动化测试验证强制计划确认、损坏包阻断、权限拒绝隔离、LinearMouse 独立确认、操作习惯动作级结果与恢复，以及异常状态不能被报告为全成功。浏览器原型没有生成真实 `.habitpack`、读取真实设备，或安装任何软件。原型结论见 [docs/product/prototype-conclusions.md](docs/product/prototype-conclusions.md)。
