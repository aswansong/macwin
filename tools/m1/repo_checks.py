...
def feature_checks(root: Path = ROOT) -> int:
    ...
    check(data["current_milestone"] == "v1.0.1-rc-release", "current milestone mismatch")
    check(data["milestone_status"] == "v1_release_candidate", "unexpected milestone status")
    ...
    check(evidence["release"].get("branch") == "release/v1.0.1", "release evidence branch mismatch")
    check(evidence["release"].get("scope") == "unsigned_v1.0.1_rc_and_release_preparation_in_progress", "release evidence scope mismatch")
    check(evidence["release"].get("decision_ref") == "D-040", "release decision evidence mismatch")
    check(evidence["release"].get("evidence_ref") == "E-047", "release evidence reference mismatch")
    return len(features)
